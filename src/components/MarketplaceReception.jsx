// ✅ Version corrigée JSX-safe pour Render/Vite
import React, { useState, useEffect, useCallback, useMemo } from 'react';
import { 
  Search, RefreshCw, Eye, Heart, MessageCircle, X, Loader, 
  Tag, MapPin, Clock, Truck, ShoppingCart, CheckCircle, AlertCircle 
} from 'lucide-react';
...
const MarketplaceReception = () => {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      const raw = Array.isArray(data) ? data : (data.commandes || data.data || []);
      const parsed = raw.map((item, idx) => ({
        id: item.id || `cmd_${idx}`,
        title: item.title || 'Produit inconnu',
        ...
        location: item.location || 'Non précisée',
        shipping: item.shipping !== false,
        user: item.user || 'Utilisateur',
        status: item.status || 'recu',
  ...
  const formatPrice = (p) => `${parseFloat(p || 0).toLocaleString('fr-FR')}€`;
  const formatDate = (d) => new Date(d).toLocaleDateString('fr-FR');

  const handleConfirmReception = async (id) => {
    try {
      await fetch(`${API_BASE}/api/commande/valider/${id}`, { method: 'POST' });
      fetchListings();
    } catch (e) {
  ...
  const filteredListings = useMemo(() => {
    return listings.filter((p) => {
      const searchable = `${p.title} ${p.brand} ${p.category}`.toLowerCase();
